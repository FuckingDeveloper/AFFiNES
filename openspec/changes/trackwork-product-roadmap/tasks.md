...
- [ ] 1.11 Add dependency vulnerability scanning to release CI.
  - Tool: OSV-Scanner v2.5.1 (`google/osv-scanner-action@v1`) against `yarn.lock`. Scope: the full resolved dependency graph. Trigger: PR + push to develop AND the release path - `.github/workflows/security-scan.yml` is reusable via `workflow_call` and `release.yml` runs it as the `security-gates` job, a required `needs` dependency of the cloud/image/desktop/mobile release jobs, so a failing gate blocks release for the EXACT commit being released. Policy: NEW Critical/High advisories fail (`.security/check-osv-baseline.py`, severity from the scanner-provided normalized label with CVSS 3.1 fallback; unclassifiable NEW severity blocks); Moderate/Low reported via the uploaded JSON artifact. Baseline identity is (ecosystem, package, resolved version, advisory): an advisory for another package/version never suppresses a new vulnerable dependency (self-tests A-E pass). Baseline `.security/osv-baseline.json` (generated 2026-09-03: 72 packages / 196 advisories - 8 Critical, 91 High, 81 Moderate, 15 Low, 1 Unknown), review/expiry 2026-12-01. Self-test: `.security/self-test/package-lock.json` (shell-quote 1.7.2) reports 3 advisories in CI; identity self-test runs in CI.
- [ ] 1.12 Add secret scanning and security-focused static analysis/linting to release CI.
  - Secret scanning: Gitleaks v8.24.3 (`gitleaks/gitleaks-action@v3`) on tracked content, PR + push to develop + release (same reusable workflow as 1.11). Allowlist in `.gitleaks.toml` is VALUE-LEVEL (exact fake values via `regexTarget: secret`); no whole-file exclusions for config source, workflows, Helm templates or auth/provider files - path exclusions exist ONLY for vendored Yarn release bundles. 0 findings on tracked content; real findings fail CI; no secrets echoed; CI self-tests: synthetic `API_KEY` detected (exit 1) + allowlist-bypass regression (`.security/gitleaks-bypass-selftest.py`) proves an unrelated synthetic secret inserted into previously sensitive files IS detected.
  - Static analysis: CodeQL (existing `analyze` job, javascript/typescript over affine + blocksuite, `security-extended` queries) now has an EXPLICIT enforceable gate: the analyze action writes local SARIF and `.security/check-codeql-sarif.py` fails the job on NEW findings with official `security-severity` >= 7.0 + precision high/very-high (or result level error) vs `.security/codeql-baseline.json` (initially empty, review backlog populated by the first CI run; unknown precision with high severity is review-required/blocking). The gate runs on PR + push and its job is in the release dependency chain; SARIF upload remains an additional reporting surface and is not required for the gate. Gate self-tests (6 cases) pass in CI.
  - Policy: `docs/security-ci.md`.
- [ ] 1.13 Add release-image/container vulnerability scanning.
  - Tool: Trivy v0.74.0 (pinned binary, SHA256SUMS-verified) scanning the ACTUAL production image built from `.github/deployment/node/Dockerfile` (node:22-bookworm-slim runtime + production workspace deps + server dist + web/admin static). Flow (`.github/workflows/build-images.yml`): ONE multi-platform candidate build pushed under the immutable candidate tag -> per-platform digests (amd64/arm64/armv7) resolved from the pushed manifest -> EVERY platform digest scanned for CRITICAL/HIGH with its own `.trivyignore-<platform>` baseline -> only after all scans pass can the caller promote/deploy; the scanned candidate digest IS the digest promoted (no rebuild after scan). SARIF reports per platform uploaded. Policy: Critical/High findings on ANY platform not in that platform's baseline fail the gate; baselines: `.trivyignore-amd64` (125 unique C/H CVEs from the 2026-09-03 local runtime-layer scan, authoritative list regenerated on the first production candidate scan), `.trivyignore-arm64`/`.trivyignore-armv7` PENDING first real candidate scans (until reviewed, ALL C/H findings on those platforms block - conservative, not pretending amd64 covers them); review/expiry 2026-12-01; unfixed CVEs reported, not silently ignored (no `--ignore-unfixed`).
  - RUNTIME VALIDATION STATUS (2026-09-03): the previous [x] states are UNSUPPORTED. GitHub-hosted CI exposed failures: the OSV GitHub Action tag `google/osv-scanner-action@v1` does not resolve (infra failure - the scanner never ran); the Gitleaks action wrapper requires GITHUB_TOKEN in PR context (infra failure); a committed literal synthetic secret in the bypass self-test was detected on push (self-test design failure); the CodeQL security-result gate found real pre-existing findings (js/clear-text-logging BLOCK, js/regex-injection REVIEW-REQUIRED, plus reproduced blocks such as js/clear-text-storage-of-sensitive-data in auth/service.ts, js/request-forgery in doc/reader.ts, js/insufficient-password-hash in byok/service.ts and 24 reviewed baseline entries) - the gate works but the findings must be explicitly triaged; the Typecheck job failure reproduces on the untouched develop content (pre-existing oauth/config.ts and admin columns.tsx type errors). Tasks may be re-checked ONLY after actual GitHub-hosted runs of the corrected workflows pass.
