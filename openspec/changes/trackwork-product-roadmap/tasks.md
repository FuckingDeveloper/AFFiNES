...
- [x] 1.6 Add archive semantics for tasks and workflow/planning entities that require history retention.
  - Current lifecycle matrix: tasks are doc-resident custom properties with a registry mirror (trackwork_tasks: workspaceId+docId+taskKey+number, unique, idempotent sync); NO task hard-delete exists - the only destructive path is the underlying AFFiNE document trash (`handleDeleteTask` -> `doc.moveToTrash()` + `task.trashed` history), which retains the doc and the registry row; document links/backlinks, development links/activity and audit rows are retained (no delete APIs); workflow boards/stages are removed via the authoritative config mutation with task remapping to a fallback board (revision + atomic audit history retains the old config); transitions/type-transitions are config rules (replace/delete is correct; history = config revisions); automation rules use `enabled=false` as the lifecycle (no archive needed); planning entities (sprint/epic/release/milestone/backlog) DO NOT EXIST as server/domain entities - documented as future Phase 6, no fictional storage invented.
