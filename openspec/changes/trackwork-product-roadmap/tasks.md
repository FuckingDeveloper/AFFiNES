...
- [x] 1.4 Define TrackWork capability permissions and replace scattered administrator-only checks where appropriate.
  - Added workspace-scoped TrackWork capabilities to the established Actions/RoleActionsMap/AccessController system: `Workspace.TrackWork.Write` (task sync/allocation registry writes; granted to Collaborator+, replacing the `Workspace.CreateDoc` proxy) and `Workspace.TrackWork.Integrations.Manage` (integration configuration, credential rotation, repository import/enable, SCM branch/MR/pipeline actions and integration reads; granted to Owner only, replacing the `Workspace.Administrators.Manage` proxy — effective access unchanged, Admin deliberately does NOT receive it). No parallel ACL, no hard-coded admin checks, no frontend-only authorization. Role mapping is documented by unit assertions and the role-permission snapshots (`core/permission/__tests__/actions.spec.ts`) and enforced server-side (proven by the cross-workspace e2e suite). Document `Doc.Read`/`Doc.Update` checks remain authoritative for individual documents.
...
- [x] 1.9 Add structured audit events for task/workflow/integration/configuration/security changes.
  - Reused the existing durable `admin_audit_logs` table and admin query (`adminAuditLogs`), extended with nullable `workspace_id` and `metadata` columns and a shared `AdminAuditService` (the user-management resolver now uses the same service). TrackWork audit events are written after successful mutations at the resolver layer (success-only convention, matching the existing `user.*` audit pattern): `trackwork.task.sync` (single bounded summary event with `taskCount`), `trackwork.task.allocate`, `trackwork.task.set_links` (`linkCount`), `trackwork.integration.create` (`provider` only), `.update`, `.delete`, `.rotate_credentials`, `trackwork.integration.import_repository`, `trackwork.integration.set_repository_enabled`, `trackwork.scm.create_branch`, `trackwork.scm.create_merge_request`, `trackwork.scm.refresh_pipelines`. Events carry actor (id+email), workspace, target id and timestamp; metadata is restricted to bounded enums and safe counts — no tokens, secrets, names, titles, urls, doc ids or user-authored content. Test connection is intentionally not audited (read/verify operation, matching the mutation-only convention). Security tests prove: authorized mutations produce events, denied/cross-workspace attempts produce no success events for the target workspace, bulk sync yields exactly one bounded record, deletion remains auditable after target removal, and no secrets or user content appear in stored records. Workflow/encryption/security mutation surfaces do not exist yet; the contract supports later categories via the same action/targetType convention.
