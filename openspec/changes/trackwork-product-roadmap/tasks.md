...
- [x] 1.14 Add targeted security integration tests for injection, XSS, SSRF, webhook replay/signature validation, file/blob authorization, abusive pagination/query complexity, and privilege escalation.
  - Security integration suite `src/__tests__/e2e/trackwork/security.spec.ts` (wired into `yarn trackwork:security` and `trackwork:check:full`):
    ...
    - XSS -> frontend surface assertions `task-tracker/security-xss.spec.ts`: no dangerouslySetInnerHTML/innerHTML/insertAdjacentHTML/document.write sinks and no javascript: hrefs in the task-tracker module; user-authored titles stay plain strings in the helpers (React escaping is the rendering boundary).
    ...
    - webhook replay -> GitLab X-Gitlab-Event-UUID deduplication implemented (narrow fix: Redis key trackwork:webhook:<connectionId>:<uuid>, 1h TTL, ack-without-reprocessing); tests prove same-uuid delivery is acked and new uuids are processed.
