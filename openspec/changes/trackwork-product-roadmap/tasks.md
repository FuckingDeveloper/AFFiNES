...
- [x] 1.11 Add dependency vulnerability scanning to release CI.
  - Tool: OSV-Scanner v2.5.1 (`google/osv-scanner-action@v1`) against `yarn.lock` with a committed baseline (`.security/osv-baseline.json`, generated 2026-09-03: 72 packages / 196 advisories - 8 Critical, 91 High, 81 Moderate, 15 Low, 1 Unknown). Scope: the full resolved dependency graph. Trigger: pull_request + push to develop + release path (via security-scan.yml). Policy: NEW Critical/High advisories fail the gate (`OSV_SCANNER_FAIL_ON_VULN` with baseline); Moderate/Low are reported via the uploaded JSON artifact. Baseline review/expiry: 2026-12-01. Self-test: `.security/self-test/package-lock.json` (shell-quote 1.7.2, GHSA-w7jw-789q-3m8p) must report findings (3 advisories) in CI. Workflow: `.github/workflows/security-scan.yml`. Accepted limitation: pre-existing baseline findings do not block; they are tracked and reviewed on the expiry date.
- [x] 1.12 Add secret scanning and security-focused static analysis/linting to release CI.
  - Secret scanning: Gitleaks v8.24.3 (`gitleaks/gitleaks-action@v3`) on tracked content; narrow file-scoped allowlist in `.gitleaks.toml` covering only fake test credentials/placeholders (39 baseline findings reviewed; 0 remain after the allowlist on tracked content); real findings fail CI; no secrets echoed; CI self-test proves a synthetic `API_KEY` is detected (exit 1). Trigger: pull_request + push to develop + release.
  - Static analysis: CodeQL (existing `analyze` job in `.github/workflows/build-test.yml`, javascript/typescript over affine + blocksuite) strengthened with `security-extended` queries; SARIF uploaded with `security-events: write`; high-confidence findings surface as GitHub security alerts per triage policy (docs/security-ci.md). On forks without Advanced Security the local analysis still runs and reports via annotations.
  - Policy: `docs/security-ci.md`.
- [x] 1.13 Add release-image/container vulnerability scanning.
  - Tool: Trivy (`aquasecurity/trivy-action@0.28.0`) on the ACTUAL self-host production image built from `.github/deployment/node/Dockerfile` (node:22-bookworm-slim runtime + production workspace deps + server dist + web/admin static). Trigger: release image build (`build-images.yml`): a scan build (linux/amd64, local load, no push) is scanned for CRITICAL/HIGH BEFORE the multi-platform push build, so the gate runs before publication; SARIF artifact uploaded. Policy: Critical/High findings not listed in `.trivyignore` fail the gate; the baseline (125 unique C/H CVEs from the 2026-09-03 local runtime-layer scan) is tracked with review/expiry 2026-12-01; unfixed CVEs are reported, not silently ignored; fixable-vs-unfixed is distinguished via `--ignore-unfixed: false` (all C/H reported) plus the tracked baseline. Accepted limitations: arm64/armv7 images are built from the same Dockerfile but only the amd64 platform build is scanned in this pass; the scan build is a separate build from the push build (documented double-build tradeoff).
