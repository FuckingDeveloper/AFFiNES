...
- [x] 1.4 Define TrackWork capability permissions and replace scattered administrator-only checks where appropriate.
  ...
  - Documented product nuance: the tracker's built-in property self-provisioning (`ensureProperty` on first open) is client-written for the same table; the built-in rows are non-essential per-device fallbacks (`BUILT_IN_CUSTOM_PROPERTY_TYPE`), so denial of collaborator sync of new rows has no visible product impact; property-schema customization remains Admin+ per RoleActionsMap.- [ ] 1.5 Add cross-workspace/object authorization tests for tasks, documents, integrations, planning entities, attachments and admin-only operations to detect IDOR regressions.
