...
- [ ] 1.4 Define TrackWork capability permissions and replace scattered administrator-only checks where appropriate.
  - Capability refactor is implemented and valid: `Workspace.TrackWork.Write` (Collaborator+) and `Workspace.TrackWork.Integrations.Manage` (Owner only) replace the `CreateDoc`/`Administrators.Manage` proxies; role mapping is documented by unit assertions/snapshots and proven server-side for GraphQL surfaces (`core/permission/__tests__/actions.spec.ts`, `src/__tests__/e2e/trackwork/idor.spec.ts`).
  - **Confirmed remaining gap**: workflow configuration (taskTrackerBoards/flow/transitions/type transitions/automation rules) is stored in the collaborative workspace-db document `db$docCustomPropertyInfo` and changed through client-side `WorkspacePropertyService` → Yjs sync. The server's sync push path (`core/sync/gateway.ts` `space:push-doc-update`) enforces only workspace membership (`Workspace.Sync`) and the blocked-doc flag; the per-document `Doc.Update` assertion is commented out. Reproduction (`src/__tests__/sync/trackwork-workflow-permission.spec.ts`) proves a Collaborator can push workflow-config updates to that document and the server accepts and persists them. Completion requires a server-side semantic boundary for workflow configuration (dedicated config mutation surface or scoped sync enforcement) — see 1.9 pending note.
...
- [ ] 1.9 Add structured audit events for task/workflow/integration/configuration/security changes.
  ...
  - Task lifecycle activity (this branch, implemented): structured client-authored activity appended to the task's collaborative `taskHistory` — additive fields `operation` (stable `TaskActivityOperation` union), `actorId`/`actorName` (session account), `taskKey`, `source` (`user`/`automation`); backward-compatible parsing of legacy records; no-op edits and neighboring reindex writes do not produce activity; documented honestly as PRODUCT ACTIVITY (client-authored, collaboration-preserving) rather than security audit. Tests: `task-tracker/config.spec.ts`.
  - Privileged workflow/config audit (still pending): workflow configuration changes happen through client-synced workspace properties (see 1.4 gap); a durable server-side audit fact (workspace, actor, operation, affected workflow configuration, server time) requires a server semantic boundary (dedicated config mutation surface or scoped sync enforcement). Client-emitted audit events would be advisory only and are not sufficient authoritative evidence.
