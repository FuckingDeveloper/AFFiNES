...
| -------------------------- | --------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Dependency vulnerabilities | OSV-Scanner (identity-scoped baseline)              | PR + push to develop + release (`.github/workflows/security-scan.yml`, reusable via `workflow_call`; release.yml `security-gates` job is a required dependency of deploy/promotion jobs) | NEW Critical/High advisories vs `.security/osv-baseline.json` scoped to (ecosystem, package, resolved version, advisory); Moderate/Low reported; unclassifiable NEW severity blocks                              |
